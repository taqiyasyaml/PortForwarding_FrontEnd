...
const RenewDialog = (props: RenewDialogProps) => {
    const { t } = useTranslation(['dashboard_user_userpackages_renewdialog', 'exception'])
    ...
    const onGetIPButtonClick = async () => {
        ...
        setIsLoading(true)
        try {
            const res = await fetch(config.BACKEND_URL + '/api/v1/user/user_package/' + encodeURI(props?.userPackageID ?? "") + '/renew', {
                method: 'post',
                headers: {
    ...
    return (<Dialog open={typeof props?.userPackageID === 'string' && props.userPackageID.length > 0} onClose={isLoading === true ? undefined : props?.onClose}>
        <DialogTitle>{t('dashboard_user_userpackages_renewdialog:RenewKeyDialogTitle')}</DialogTitle>
        <DialogContent>
            <DialogContentText sx={{ display: 'inline' }}>
                {t('dashboard_user_userpackages_renewdialog:MessagePrefixDialogContentText') + ' '}
                <Typography fontWeight={(t: Theme) => t.typography.fontWeightMedium} display="inline">
                    {props?.userPackageID ?? ""}
                </Typography>
                {' ' + t('dashboard_user_userpackages_renewdialog:MessagePostfixDialogContentText')}
            </DialogContentText>
        ...
        <DialogActions>
            <Button onClick={isLoading === true ? undefined : props?.onClose}>{t('dashboard_user_userpackages_renewdialog:CloseButtonDialogActions')}</Button>
            {isLoading === true ? (
                ...
                >{t('dashboard_user_userpackages_renewdialog:SubmitButtonDialogActions')}</Button>
            )}
        </DialogActions>
    </Dialog >)
}
